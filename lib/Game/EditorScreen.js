	...
	Game.EditorScreen = (function() {
			...
			insert() {
				...
				if (el instanceof Framework.Mapping.Map) {
					...
					if(tilePos.x !== this.lastTilePos.x || tilePos.y !== this.lastTilePos.y){
						...
						el.replaceTerrainTile(tilePos.x, tilePos.y, Runtime.screen.insertTile);
					}
				}
			}
		}
		...
		class EditorScreen extends Framework.Screen {
			constructor(options) {
				...
				this.map = new Framework.Mapping.Map({tileDefinitionFile:'asset/Terrain/Summer.json', tileCountHorizontal:200,tileCountVertical:200, cb:editorScreen_MapLoaded.bind(this)});
				...
				this.mudmenu.mouseClick = function() {
					Runtime.screen.insertTile = Game.Tiles.Type.Mud;
				};
				...
				this.shallowsmenu.mouseClick = function() {
					Runtime.screen.insertTile = Game.Tiles.Type.Shallows;
				};
