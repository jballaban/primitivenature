...
Global.loader.load(['lib/Framework/Element.js'], function() {
	Game.Tiles = {};
	Game.Tiles.Summer = (function() {
		class Summer extends Framework.Element {
			constructor(options) {
				super(options);
			...
			_getSprite(connections) {
				for (var i=0;i<this.set.length;i++) {
					if (Game.Tiles.Type.equals(this.set[i].connections, connections)) {
						var sourceX = this.set[i].sourceX + (Math.floor(Math.random()*this.set[i].length));
			...
			_setConnection(tile, positions, type) {
				...
				tile.sprite = this._getSprite(tile.connections) || new Framework.Sprite.Rectangle({ colour: '#F00', fill: true, radius: 0 });
			}
			...
			_getSet() {
				...
				var set = [
					// long grass
					{ connections: [L, L, 
									L, L], sourceX: 3, sourceY: 19, length: 2 },
					{ connections: [L, G, 
	...
	Game.Tiles.Tile = (function() {
		class Tile extends Framework.Element {
			constructor(options) {
				super(options);
			...
			update() {
				if (this.hovered && Framework.MouseHandler.mouse.leftButtonPressed) {
					Runtime.screen.insert();
				}
			}
