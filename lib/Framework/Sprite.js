		...
		class SpriteImage {
			...
			getContext(leaveblank){
				...
				if(this.canvas === null){
					...
					this.img = null; //unload image
				}else{
					//canvas already exists, it's likely already not blank, and even if it is, leaveblank calls are expected to redraw entire canvas.
					//Thus ignore leaveblank parameter
					ctxt = this.canvas.getContext('2d');
				}
			...
			draw(x, y, w, h, a) {
				...
				if (this.frames.length === 0)
					Runtime.canvas.drawImage(this.canvas || this.img, x, y, w, h);
				else {
					Runtime.canvas.drawImage(this.canvas || this.img, this.frames[this.frame].x, this.frames[this.frame].y, this.frames[this.frame].w || w, this.frames[this.frame].h || h, x, y, w, h);
				}
			}
		}
		return SpriteImage;
