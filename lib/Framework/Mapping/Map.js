...
Global.loader.load(['lib/Framework/Framework.js','lib/Framework/Element.js','lib/Framework/Mapping/Tileset.js','lib/Framework/Sprite.js'],function(){
	...
	var replaceTileAndFlattenTerrain = function(map, newSlots, pos){
		var overlayTransitions = createTransitionMap(map.tileCountHorizontal,map.tileCountVertical),
			...
			spiralManager = new TransitionSpiralOut(pos.x,pos.y,map.tileCountHorizontal, map.tileCountVertical),
			cPos,expireSpiral = 1,
			getOverlayedVal = function(x,y){
				if(overlayTransitions[x][y]===GOD_TILESLOT)
					return originalTransitions[x][y];
				return overlayTransitions[x][y];
			},
			spiralOutProcess = function(){
				while((cPos = spiralManager.getNextCell()) !== null){
					if(expireSpiral === spiralManager.spiral){
						console.log('spiral manager pre-empted')
						break;
					}
		...
		for(let x = 0; x < overlayTransitions.length-1; x++){
			for(let y = 0; y< overlayTransitions[x].length-1;y++){
				var cTrans = overlayTransitions[x][y] || overlayTransitions[x+1][y] || overlayTransitions[x][y+1] || overlayTransitions[x+1][y+1];
				if(cTrans!==null){
					map.terrain[x][y] = map.tileset.searchForTilesBySlotsHighestSpecificity([
						getOverlayedVal(x+1,y),
						getOverlayedVal(x+1,y+1),
						getOverlayedVal(x,y+1),
						getOverlayedVal(x,y)
					],false).getTile();
					map.invalidate(x,y);
				}
	...
	class Map extends Framework.Element{
		...
		invalidate(x,y){
			...
			if(typeof(x) !== 'undefined'){
				this._invalidatedTiles.push({x:x,y:y});
			}
		}
		...
		draw(){
			...
			if(this.invalidated){
				...
				if(this._invalidatedTiles.length===0){
					for(let y = 0; y < this.tileCountVertical; y++){
						for(let x = 0; x < this.tileCountHorizontal; x++){
					...
					for(let x = 0; x < this._invalidatedTiles.length; x++){
						let ctc = this._invalidatedTiles[x]; //Current Tile Coordinates
						this.terrain[ctc.x][ctc.y].drawTo(ctxt,tileSize.x*ctc.x, tileSize.y*ctc.y);
					}
					this._invalidatedTiles=[];
				}
				this.invalidated = false;
			}
			Runtime.canvas.drawImage(this.img.getDrawable(),0,0);
		}
	}
