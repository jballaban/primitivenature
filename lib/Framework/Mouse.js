...
Framework.MouseHandler = new JS.Singleton({
	...
	update: function(screen) { // current Framework.Screen
		...
		if (newfocus != this.focus) {
			...
			this.focus.getMouse().move(this.x, this.y);
		}
		this.focus.getMouse().update();
	},
	draw: function() {
		if (this.focus != null) {
			this.focus.getMouse().draw();
		}
	},
	mouseUp: function(e) {
		if (this.focus != null)
			this.focus.getMouse().up();
	},
	mouseMove: function(e) {
		this.x = e.clientX;
		this.y = e.clientY;
		if (this.focus != null)
			this.focus.getMouse().move(e.clientX, e.clientY);
	},
	mouseDown: function(e) {
		e.preventDefault();
		this.focus.getMouse().down();
		//this.x = e.clientX;
		//this.y = e.clientY;
		//this.height = 1;
		//this.width = 1;
