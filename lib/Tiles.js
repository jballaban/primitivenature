...
Game.Tiles.Tileset = new JS.Class(Framework.Element, {
	...
	insert: function(tileX, tileY, type, ignoretype) {
		...
		el.setConnection([0,1,2,3], type);
		if (adjacent[1-1] != null)
			adjacent[1-1].setConnection([1], type);
		if (adjacent[2-1] != null)
			adjacent[2-1].setConnection([0,1], type);
		if (adjacent[3-1] != null)
			adjacent[3-1].setConnection([0], type);
		if (adjacent[4-1] != null)
			adjacent[4-1].setConnection([1,3], type);
		if (adjacent[6-1] != null)
			adjacent[6-1].setConnection([0,2], type);
		if (adjacent[7-1] != null)
			adjacent[7-1].setConnection([3], type);
		if (adjacent[8-1] != null)
			adjacent[8-1].setConnection([2,3], type);
		if (adjacent[9-1] != null)
			adjacent[9-1].setConnection([2], type);
	},
...
Game.Tiles.Tile = new JS.Class(Framework.Element, {
	initialize: function(options) {
		...
		this.callSuper();
		this.setConnection([0,1,2,3], Game.Tiles.Type.Dirt);
		this.hoversprite = new Framework.Sprite.Rectangle({ colour: '#FF0',  alpha:0.1, fill: true, radius: 18 });
	},
	setConnection: function(positions, type) {
		for (var i=0;i<positions.length;i++)
			this.connections[positions[i]] = type;
		

		this.sprite = this.getSprite() || new Framework.Sprite.Rectangle({ colour: '#F00', fill: true, radius:0 });
	},
	draw: function() {
		this.callSuper();
		if (this.hovered)
	...
	getSprite: function() {
		...
		for (var i=0;i<selections.length;i++) {
			if (Game.Tiles.Type.equals(selections[i].connections, this.connections)) {
				var sourceX = selections[i].sourceX += (Math.floor(Math.random()*selections[i].length));
				var sourceY = selections[i].sourceY;
				while (sourceX >= 19) {
					sourceX=sourceX-19;
					sourceY+=1;
				}
		...
		console.log('no sprite found ', this.connections);
		return null;
	}
});